...
import {useHiveContext} from "./HiveProvider";
...
function CustomMarker({ hiveNumber }) {
    return (
        <View
            style={{
                alignItems: 'center',
            ...
            <View style={{// You can adjust this value for desired roundness
            }}>
                <Image
                    source={require('./assets/images/hivemarker.png')}
                    style={{ width: 30, height: 30 }}
                />
            </View>
            <View style={{
                position: 'absolute',
                bottom: 0, // adjust this value to position the number as you like
                width: 20,
                height: 20,
                borderRadius: 10,
...
export default function PriorityMap() {
    ...
    const {selectedHives, setSelectedHives} = useHiveContext();
    ...
    const fetchDistances = async () => {
        ...
        let origin = `${originLocation[0].location.latitude},${originLocation[0].location.longitude}`;

        for (let i = 0; i < hiveCoords.length; i++) {
            const hive = hiveCoords[i];
            ...
            try {
                ...
                // Set the current destination as the next origin
                origin = dest;
    ...
    const renderHives = () => {
        ...
        return selectedHives.map((hive, index) => {
            ...
            return (
                <Marker
                    key={id}
                    coordinate={{
                        latitude: hive.location.latitude,
                        longitude: hive.location.longitude,
                    }}>
                    <CustomMarker hiveNumber={index + 1} />
                </Marker>
            );
        });
    };
    ...
    const addHive = (coordinates) => {
        ...
        setSelectedHives((prevHives) => [...prevHives, newHive]);
        setCount(count + 1);
    };
