...
function HiveStrengthList() {
    ...
    const {setSelectedHives, selectedHives} = useHiveContext(); // Destructure selectedHives from context
    const [selectedCells, setSelectCells] = useState([]);
    ...
    useEffect(() => {
        async function fetchHives() {
            try {
                const response = await axios.get(`http://130.157.220.43:8000/hives`);
                    ...
                    sortedHives.map(async (hive) => {
                        try {
                            const taskResponse = await axios.get(
                                `http://130.157.220.43:8000/tasks/${hive.tasks_id}`
                            );
    ...
    const handleSelectHive = (hive) => {
        if (!selectedHives.includes(hive)) {
            setSelectedHives((prevHive) => [...prevHive, hive]);
            setSelectCells((hives) => [...hives, hive]);
        }
    };
    ...
    const renderItem = ({item}) => {
        ...
        const isSelected = selectedCells.includes(item);
        count += 1;

        return (
            <TouchableOpacity onPress={() => handleSelectHive(item)}>
    ...
    return (
        <View style={styles.container}>
            <FlatList data={allHives} renderItem={renderItem} key={count}/>
        </View>
    );
}
